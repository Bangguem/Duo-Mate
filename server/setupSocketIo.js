...
const setupSocketIo = (server) => {
  ...
  class MatchQueue {
    ...
    findMatchablePlayers(player) {
      ...
      return this.queue.filter((entry) => {
        ...
        const sr1 =
          Array.isArray(player.user.summonerRank) &&
          player.user.summonerRank[0];
        const sr2 =
          Array.isArray(entry.user.summonerRank) && entry.user.summonerRank[0];
        if (!sr1 || !sr2) return false;

        // duoRestrictions 의 키가 "TIER RANK" 형식인지 "TIER" 형식인지
        // 동적으로 결정
        const key1 = sr1.rank ? `${sr1.tier} ${sr1.rank}` : sr1.tier;
        const key2 = sr2.rank ? `${sr2.tier} ${sr2.rank}` : sr2.tier;

        // duoRestrictions[key1]가 없으면, tier 단독 키로도 시도
        const allow1 = duoRestrictions[key1] || duoRestrictions[sr1.tier] || [];
        const allow2 = duoRestrictions[key2] || duoRestrictions[sr2.tier] || [];

        // 서로의 key가 허용 리스트에 있는지 확인
        return allow1.includes(key2) || allow2.includes(key1);
      });
    }
    ...
    processBatch() {
      ...
      for (let i = 0; i < this.queue.length; i++) {
        ...
        const sr1 =
          Array.isArray(player1.user.summonerRank) &&
          player1.user.summonerRank.length > 0
        ...
        // 자기 자신 제외 및 랜덤 선택
        const validPlayers = matchablePlayers.filter(
          (p) =>
        ...
        const sr2 =
          Array.isArray(player2.user.summonerRank) &&
          player2.user.summonerRank.length > 0
        ...
        console.log(
          `✅ 매칭 성공: ${player1.user.nickname}(${tier1}) - ${player2.user.nickname}(${tier2})`
        );
        processed.add(i);
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("join room", ({ roomName }) => {
      ...
      console.log(
        `📊 방 ${roomName}의 현재 사용자 수: ${room ? room.size : 0}`
      );
    });
    ...
    socket.on("chat message", ({ matchId, message, timestamp }) => {
      ...
      if (!match) {
        console.error(
          `❌ 매치 ID ${matchId}에 대한 매칭 정보를 찾을 수 없습니다.`
        );
      ...
      if (!sender) {
        console.error(
          `❌ 소켓 ID ${socket.id}에 대한 플레이어 정보를 찾을 수 없습니다.`
        );
        console.log("📊 현재 플레이어 목록:", match.players);
        const senderByUserId = match.players.find(
    ...
    socket.on("request rankmatch", async ({ position, microphone }) => {
      try {
        ...
        if (!user.summonerInfo) {
          return socket.emit("matchError", {
            message: "랭크 매칭을 위해 소환사 정보를 등록해주세요.",
          });
        }
        ...
        if (
          !user.summonerRank ||
          !Array.isArray(user.summonerRank) ||
          user.summonerRank.length === 0
        ) {
          return socket.emit("matchError", {
            message: "랭크 매칭을 위해 랭크 정보가 필요합니다.",
          });
        }
    ...
    socket.on("leave room", ({ matchId, userId, nickname }) => {
      ...
      console.log(
        `✅ 사용자 ${nickname}가 방 ${match.roomName}에서 나갔습니다.`
      );
    });
